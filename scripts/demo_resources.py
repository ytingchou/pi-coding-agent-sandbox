...
async def main(live=False, keep=False):
    async with httpx.AsyncClient(
        ...
        timeout=600,
        headers={"Authorization": "Bearer " + os.getenv("API_TOKEN", "local-demo-change-me")},
    ) as client:
        ...
        try:
            for worker in ("sandbox-1", "sandbox-1", "sandbox-2"):
                sessions.append(
                    await call("POST", f"/agents/{agent}/sessions", {"sandbox_id": worker})
                )
            ...
            result = await call(
                "POST", paths[0] + "/pi/prompt", {"prompt": "/package-stats 2,4,6,8"}
            )
            ...
            after_reload = await call(
                "POST", paths[0] + "/pi/prompt", {"prompt": "/package-stats 10,20"}
            )
            ...
            if live:
                ...
                show(
                    "Live skill writes and executes Python",
                    await call(
                        "POST", paths[0] + "/pi/prompt", {"prompt": "/skill:python-stats 2,4,6,8"}
                    ),
                )
                show(
                    "Live Agents SDK → pi package tool",
                    await call(
                        "POST",
                        f"/agents/{agent}/run",
                        {
            ...
            print(
                "PASS: native pi remove unloads the package; standalone resources remain",
                flush=True,
            )
            if not live:
                print(
                    "Offline demo complete. Skill model execution and Agents SDK model turns require --live.",
                    flush=True,
                )
        finally:
            if not keep:
                ...
                print(
                    "Sessions retained (--keep). Use their IDs to continue or delete them.",
                    flush=True,
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--live", action="store_true", help="Call real models; requires OPENAI_API_KEY"
    )

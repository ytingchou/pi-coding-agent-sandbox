...
pytestmark = pytest.mark.skipif(
    os.getenv("RUN_ISOLATION_TESTS") != "1", reason="Run inside the sandbox test container"
)
...
async def test_real_pi_sessions_are_isolated_and_reconnect(tmp_path):
    ...
    try:
        ...
        for sid in (a, b):
            await bash(
                sid,
                "python -c 'import sys; assert sys.version_info[:2] == (3,12); assert sys.prefix == \"/workspace/venv\"' && python3 -c 'import sys; assert sys.version_info[:2] == (3,12)' && pip --version && uv --version",
            )
...
"""
        await bash(
            ...
            "python -c "
            + shlex.quote(install)
            + " && pip install --no-index /workspace/session_only-1.0-py3-none-any.whl && python -c 'import session_only; assert session_only.VALUE == 42'",
        )
        await bash(
            a,
            "uv pip install --offline /workspace/uv_session_only-1.0-py3-none-any.whl && python -c 'import uv_session_only; assert uv_session_only.VALUE == 42'",
        )
...
"""
        ...
        results = await asyncio.gather(
            bash(a, "python -c 'print(111)'"), bash(b, "python -c 'print(222)'")
        )
        assert "111" in results[0] and "222" in results[1]
        await worker.close()
        worker = Worker(tmp_path / "state", root)
        assert "42" in await bash(
            a,
            "python result.py && python -c 'import session_only, uv_session_only; assert uv_session_only.VALUE == 42; print(session_only.VALUE)'",
        )

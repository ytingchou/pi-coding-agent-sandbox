...
async def test_native_resources_lifecycle_and_isolation(tmp_path):
    ...
    try:
        ...
        assert "python_stats" in json.loads((await worker.prompt(a, "/session-info"))["output"])["tools"]
        ...
        assert "SESSION_A_ONLY" in await bash(a, "cat /workspace/home/.pi/agent/skills/python-stats/SKILL.md")
        assert "SESSION_A_ONLY" not in await bash(b, "cat /workspace/home/.pi/agent/skills/python-stats/SKILL.md")
        ...
        assert "python_stats" not in json.loads((await worker.prompt(a, "/session-info"))["output"])["tools"]
    finally:
        await worker.close()

        ...
        self.process = await asyncio.create_subprocess_exec(
            *isolation.command(
                ...
                [
                    ...
                    isolation.BOOTSTRAP,
                    "pi-bootstrap",
                    os.getenv("PI_MODEL", "gpt-4.1-mini"),
                ],
            ),
        ...
        try:
            while line := await self.process.stdout.readline():
                ...
                if event.get("type") == "response":
                    ...
                    event.get("type") == "extension_error"
                    and self.turn
                    and len(self.extension_errors) < 32
                ):
    ...
    async def request(self, kind, timeout=10, **payload):
        ...
        try:
            self.process.stdin.write(
                (json.dumps({"id": request_id, "type": kind, **payload}) + "\n").encode()
            )
        ...
        try:
            ...
            async with asyncio.timeout(timeout):
                ...
                if prompt.startswith("/"):
                    ...
                    extension_command = any(
                        c["name"] == name and c["source"] == "extension"
                ...
                output = "\n".join(
                    c.get("text", "") for c in last.get("content", []) if c.get("type") == "text"
                )

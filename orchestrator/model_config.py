...
def configured_model():
    ...
    if parsed.scheme not in ("http", "https") or not parsed.netloc or parsed.username or parsed.password or parsed.query or parsed.fragment:
        raise ValueError("OPENAI_BASE_URL must be an HTTP(S) base URL without credentials, query or fragment")
    mode = os.getenv("OPENAI_API_MODE") or ("chat_completions" if os.getenv("OPENAI_BASE_URL") else "responses")
    ...
    return adapters[mode](model=os.getenv("OPENAI_MODEL") or "gpt-4.1-mini", openai_client=client), client

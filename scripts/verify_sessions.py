...
def docker_python(service, code, payload, python='python'):
    result = subprocess.run(['docker','compose','exec','-T',service,python,'-c',code],
                            input=json.dumps(payload),text=True,capture_output=True,cwd=ROOT,timeout=700)
...
def usage_summary(report):
    ...
    for q in report.get('requests',[]):
        ...
        if u is None:
            ...
            for k in outer: outer[k] += u[k]
    for e in report.get('evidence',[]):
        for k in pi: pi[k] += e['usage'][k]
    ...
    if not report.get('fresh_sessions'):
        gaps.append('Existing sessions: Pi is lifetime usage; outer totals include only captured requests')
    total = outer['total_tokens']+pi['totalTokens']
    return {'agents_sdk':outer,'pi':pi,'recorded_total_tokens':total,
            'complete_for_this_run':not gaps,'limitations':gaps,
...
def collect(report, target, artifacts):
    ...
    for s in report['sessions']:
        validate_session(s)
        data = docker_python(s['sandbox_id'],COLLECT_CODE,{**s,'artifacts':artifacts},'python3')
        ...
        for name, content in data['files'].items():
            ...
            os.chmod(dest,0o600)
        rows = [json.loads(line) for line in data['files'].get('state/session.jsonl','').splitlines() if line.strip()]
        messages = [v['message'] for v in rows if v.get('type')=='message']
        assistants = [m for m in messages if m.get('role')=='assistant']
        usage = {k:sum(m.get('usage',{}).get(k,0) for m in assistants)
                 for k in ['input','output','cacheRead','cacheWrite','totalTokens']}
        gaps = ([] if 'state/session.jsonl' in data['files'] else ['Session transcript not persisted or missing: '+s['id']])
        gaps += ['Assistant missing usage' for m in assistants if not m.get('usage')]
        gaps += ['Assistant ended with error/aborted; provider may have unreported usage'
                 for m in assistants if m.get('stopReason') in ('error','aborted')]
        trace = []
        for row in rows:
            m = row.get('message',{})
            if m:
                trace.append({'entry_id':row.get('id'),'parent_id':row.get('parentId'),
                              'timestamp':row.get('timestamp'),'message':m})
        save(folder/'trace.json',trace)
        history = docker_python('api',HISTORY_CODE,s)
        save(folder/'outer-history.json',history)
        evidence.append({**s,'model':data['model'],'usage':usage,'usage_gaps':gaps,
                         'assistant_messages':len(assistants),'missing_files':data['missing'],
                         'files':data['files'],'trace':trace})
...
def verify_evidence(report):
    ...
    def calls(e):
        return {v['name'] for row in e['trace'] for v in row['message'].get('content',[])
                if isinstance(v,dict) and v.get('type')=='toolCall'}
    def successful(e, name):
        return any(row['message'].get('role')=='toolResult' and row['message'].get('toolName')==name
                   and not row['message'].get('isError',False) for row in e['trace'])
    for e in (a,b):
        assert {'write','bash'} <= calls(e), 'Missing actual write/bash calls'
        assert successful(e,'write') and successful(e,'bash'), 'Missing successful write/bash results'
    assert 'sum' in a['files']['verification.py']
    assert any('55' in str(row['message'].get('content')) for row in a['trace']
    ...
    assert any('<skill name="python-stats"' in str(row['message'].get('content')) for row in b['trace'])
    assert any('<skill name="package-stats"' in str(row['message'].get('content')) for row in c['trace'])
    ...
    report['checks'] += ['Python write → bash execution → outer Agent result',
...
def verify(report, target, args):
    ...
    call(report,target,'POST',f"/agents/{a['agent_id']}/run",{
        'session_ids':[a['id']], 'prompt':'Ask pi to write /workspace/verification.py that computes sum of squares of integers 1 through 5, then execute it with python. Return the actual numeric result concisely.'})
    print('Python delegation finished.',flush=True)
    call(report,target,'POST',session_path(b)+'/pi/prompt',{'prompt':'/skill:python-stats 3,6,9'})
    print('Standalone skill finished.',flush=True)
    call(report,target,'POST',session_path(c)+'/packages',{'action':'install','source':'/workspace/packages/stats-kit'})
    call(report,target,'POST',f"/agents/{c['agent_id']}/run",{
        'session_ids':[c['id']], 'prompt':"Delegate to pi with the exact prompt '/skill:package-stats 3,6,9'. Ask pi to also call session_info. Return its actual tool results concisely."})
    for s in (a,b):
...
def main():
    ...
    v=sub.add_parser('verify',help='Run three live model scenarios; sessions retained for inspection')
    v.add_argument('--output',default='artifacts/verification-'+dt.datetime.now().strftime('%Y%m%d-%H%M%S'))
    v.add_argument('--worker1',default='sandbox-1');v.add_argument('--worker2',default='sandbox-2')
    ...
    for p in (v,sub.choices['collect']):
        p.add_argument('--artifact',action='append',default=DEFAULT_ARTIFACTS.copy(),help='Additional UTF-8 workspace-relative file')
    args=parser.parse_args()
    for name in getattr(args,'artifact',[]):
        p=Path(name)
        if p.is_absolute() or '..' in p.parts or not p.parts or p.parts[0] in ('home','state','venv'):
    ...
    if args.command=='verify':
        ...
        report={'started_at':now(),'fresh_sessions':True,'status':'running','sessions':[],'requests':[],'checks':[]}
        save(target,report)
        try:
            verify(report,target,args)
        except Exception as exc:
            report['status']='failed';report['error']=str(exc);save(target,report)
            try: collect(report,target,args.artifact)
            except Exception as collection_error:
                report['collection_error']=str(collection_error)
                report['token_summary']=usage_summary(report);save(target,report)
            print(f'FAILED: {exc}. No automatic retry or deletion. Report: {target}',file=sys.stderr)
            return 1
    else:
        target=Path(args.report).resolve();report=json.loads(target.read_text())
        if args.command=='collect':collect(report,target,args.artifact)
        else:
            ...
            for s in report['sessions']:
                ...
                if not s.get('deleted_at'):
                    call(report,target,'DELETE',session_path(s));s['deleted_at']=now();save(target,report)

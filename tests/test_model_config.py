...
@pytest.mark.asyncio
@pytest.mark.parametrize('mode,adapter',[('chat_completions',OpenAIChatCompletionsModel),('responses',OpenAIResponsesModel)])
...
def test_pi_custom_models_and_secret_reference(tmp_path,monkeypatch):
    ...
    env={'PI_BASE_URL':'http://gateway.internal/v1','PI_MODEL':'arbitrary-company-model',
         'PI_API_KEY':'do-not-store-this-key','PI_API_MODE':'chat_completions'}
    ...
    env['PI_API_MODE']='responses';configure(tmp_path,env)
...
def test_invalid_modes_fail_before_network(tmp_path,monkeypatch):
    ...
    with pytest.raises(ValueError,match='OPENAI_API_MODE'):configured_model()

    ...
    async with httpx.AsyncClient(
        ...
        try:
            for target in ("sandbox-1", "sandbox-1", "sandbox-2"):
                sessions.append(
                    await call("POST", f"/agents/{agent}/sessions", {"sandbox_id": target})
                )
            assert len(await call("GET", f"/agents/{agent}/sessions")) == 3
            for session in sessions:
                assert (await call("POST", f"/agents/{agent}/sessions/{session['id']}/connect"))[
                    "status"
                ] == "ready"
            ...
            print(
                "PASS: two healthy workers, three Pi sessions, reconnect, ownership, authentication"
            )
        finally:
            for session in sessions:

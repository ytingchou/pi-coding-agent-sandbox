...
class SessionManager:
    def __init__(self, state=Path("/state"), endpoints=None, client=None):
        ...
        self.client = client or httpx.AsyncClient(
            headers={"Authorization": "Bearer " + os.getenv("SANDBOX_TOKEN", "worker-demo-change-me")},
    ...
    def sessions(self, agent_id):
        self.agent(agent_id)
        return [dict(row) for row in self.db.execute("SELECT * FROM bindings WHERE agent_id=? ORDER BY rowid", (agent_id,))]

    def binding(self, agent_id, session_id):
        self.agent(agent_id)
        row = self.db.execute("SELECT * FROM bindings WHERE id=? AND agent_id=?", (session_id, agent_id)).fetchone()
        if not row:
    ...
    async def call(self, sandbox_id, method, path, **kwargs):
        ...
        try:
            ...
            raise HTTPException(504, "Sandbox connection timed out; request was not retried and may have executed") from exc
        except httpx.RequestError as exc:
            raise HTTPException(503, "Sandbox unavailable; existing sessions are never moved silently") from exc
    ...
    async def allocate(self, agent_id, sandbox_id=None):
        ...
        if sandbox_id is None:
            ...
            counts = dict(self.db.execute("SELECT sandbox_id, count(*) FROM bindings GROUP BY sandbox_id"))
        ...
        with self.db:
            self.db.execute("INSERT INTO bindings VALUES (?, ?, ?, 'allocating')", (session_id, agent_id, sandbox_id))
        try:
            await self.connect(agent_id, session_id)
        except HTTPException as exc:
            # Keep uncertain allocations addressable for explicit reconnect/delete.
            raise HTTPException(exc.status_code, {"message": exc.detail, "session_id": session_id, "sandbox_id": sandbox_id}) from exc
    ...
    async def prompt(self, agent_id, session_id, prompt):
        ...
        return await self.call(binding["sandbox_id"], "POST", f"/sessions/{session_id}/prompt", json={"prompt": prompt})
    ...
    async def resource_call(self, agent_id, session_id, method, path, **kwargs):
        ...
        if binding["status"] != "ready":
            raise HTTPException(409, "Session not ready; reconnect first")
        return await self.call(binding["sandbox_id"], method, f"/sessions/{session_id}/{path}", **kwargs)

    async def health(self):
        async def check(name):
            try:
                response = await self.client.get(self.endpoints[name].rstrip("/") + "/health", timeout=3)

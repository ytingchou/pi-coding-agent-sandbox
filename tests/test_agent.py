    ...
    async def get_response(self, *args, **kwargs):
        ...
        if self.calls == 1:
            output = [ResponseFunctionToolCall(
                ...
                arguments=json.dumps({"session_id": self.session_id, "prompt": "Write Python to compute 6 * 7 and run it"}),
            )]
        else:
            assert "42" in str(kwargs.get("input", args))
            output = [ResponseOutputMessage(
                id="msg_test", type="message", role="assistant", status="completed",
                content=[ResponseOutputText(type="output_text", text="Result: 42", annotations=[])],
            )]
...
async def test_reject_foreign_tool_session(tmp_path):
    ...
    with pytest.raises(Exception, match="Session is not authorized"):
        await run_agent(Manager(), "a", "compute", model=ScriptedModel("foreign"), history_path=str(tmp_path / "h.db"))

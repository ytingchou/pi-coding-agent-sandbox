    ...
    async with httpx.AsyncClient(
        ...
        async def post(path, payload):
            ...
            if path.endswith("/run"):
                executions = [
                    tool
                    for item in result["sandbox_results"]
        ...
        sessions = [
            await post(f"/agents/{agent}/sessions", {"sandbox_id": name})
        ...
        for session, task in zip(sessions, tasks, strict=True):
            result = await post(
                f"/agents/{agent}/run", {"prompt": task, "session_ids": [session["id"]]}
            )
            print(json.dumps(result, indent=2, ensure_ascii=False), flush=True)
        result = await post(
            f"/agents/{agent}/run",
            {
                "prompt": "Read result.txt from the earlier run using Python and multiply it by 2. Execute the code.",
                "session_ids": [sessions[0]["id"]],
            },
        )
        print(json.dumps(result, indent=2, ensure_ascii=False))
        print(
            "Sessions retained for inspection. Use the documented DELETE endpoints to remove them."
        )

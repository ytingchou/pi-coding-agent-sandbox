...
spec = importlib.util.spec_from_file_location(
    "verify_sessions", ROOT / "scripts/verify_sessions.py"
)
...
class AccountingTests(unittest.TestCase):
    ...
    def test_real_evidence_passes_but_removed_execution_fails(self):
        ...
        if not source.exists():
            self.skipTest("Historical real-run fixture unavailable")
        r = json.loads(source.read_text())
        r["evidence"] = [
            {**s, "id": s["session_id"], "trace": [{"message": m} for m in s["messages"]]}
            for s in r["sandbox_evidence"]
        ]
        v.verify_evidence(r)
        broken = copy.deepcopy(r)
        broken["evidence"][0]["trace"] = [
            row
            for row in broken["evidence"][0]["trace"]
            if row["message"].get("toolName") != "bash"
        ]
        with self.assertRaises(AssertionError):
            v.verify_evidence(broken)
